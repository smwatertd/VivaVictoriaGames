...
class QuestionsClient:
    ...
    async def random_by_category(self, category_id: int) -> Question:
        ...
        answers = [Answer(id=answer.id, body=answer.body) for answer in response.answers]
        [correct_answer] = [Answer(id=answer.id, body=answer.body) for answer in response.answers if answer.is_correct]
